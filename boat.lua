...
binds.main=game:GetService"RunService".Heartbeat:Connect(function()
	...
	pcall(function()
		...
		if t==300 then
			toggle.farm=false
			notif("stopped","5 minutes has passed",math.huge)
		end
	end)
end)
...
binds.rspwn=plr.CharacterAdded:Connect(function(v)
	...
	if toggle.farm then
		...
		repeat task.wait()until hrp.CFrame.z>=dbg.z
...
local Configs=UI:addPage("Configs",1,false,1)
...
Configs:addTextBox("pfs",dbg.pfs,function(v)
	if"number"==type(tonumber(v))then
		local p=workspace:FindFirstChild"conveyor"
		p.Velocity=p.CFrame:vectorToWorldSpace(Vector3.new(0,0,tonumber(v)))
	end
end)
Configs:addTextBox("z",dbg.z,function(v)
	if"number"==type(tonumber(v))then
		dbg.z=tonumber(v)
	end
end)
