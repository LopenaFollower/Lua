		...
		if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
			...
			input.Changed:Connect(function()
				if input.UserInputState==Enum.UserInputState.End then
					dragging=false
					if gui.Position.X.Scale>workspace.CurrentCamera.ViewportSize.X-5 then
						gui.Position=UDim2.new(gui.Position.X.Scale-5,0,gui.Position.Y.Scale,0)
					end
					if gui.Position.Y.Scale>workspace.CurrentCamera.ViewportSize.Y-5 then
						gui.Position=UDim2.new(gui.Position.X.Scale,0,gui.Position.Y.Scale-5,0)
					end
				end
			end)
		end
