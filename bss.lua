...
local Welcome = Instance.new("TextLabel")
 local def_gvt = game.Workspace.Gravity
wait(.5)
--Properties:
...
local function goto(part_pos,speed)
	while(part_pos-game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude>=5 and wait()and part_pos do
		game.Players.LocalPlayer.Character:TranslateBy((part_pos-game.Players.LocalPlayer.Character.HumanoidRootPart.Position)*(speed or 0.95))
	end
end
...
KillTunnel.MouseButton1Down:connect(function(j)
      ...
      game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(7.5,20.5,0)
...
if key == "o" then
noclip = not noclip
game.Players.LocalPlayer.Character.HumanoidRootPart.Anchored = noclip
end
end)
...
Killcoconutcrab.MouseButton1Down:connect(function()
      ...
      game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-256.375092, 95, 500.457794, 0.998952866, -0.0450557806, 0.00794458669, -8.64538929e-09, 0.173648775, 0.98480767, -0.0457508452, -0.98377645, 0.173466951)
else
      game.Workspace.Gravity=def_gvt
      killcoconut = false
      noclip = false
...
local jimmy = coroutine.wrap(function()
        ...
        for k,v in pairs(workspace.Collectibles:GetChildren()) do
            if tostring(v) == tostring(game.Players.LocalPlayer.Name) or tostring(v) == "C" then
                        if (v.Position-sanghuman.Position).magnitude <= 80 then
                goto(Vector3.new(v.Position.x, sanghuman.Position.y, v.Position.z))
...
chr:TranslateBy(hum.MoveDirection * tonumber(args))
end end end
end)
Voiddz(sanghuman,sangzboi)
jimmy3()   
